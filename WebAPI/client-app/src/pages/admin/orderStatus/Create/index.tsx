...
const OrderStatusCreate: FC<CreateProps> = ({ afterCreate }) => {
    const { t } = useTranslation();

    const [open, setOpen] = useState(false);

    const { CreateOrderStatus } = useActions();

    const item: IOrderStatus = {
        englishName: "",
        ukrainianName: "",
    };
    ...
    const onHandleSubmit = async (values: IOrderStatus) => {
        try {
            await CreateOrderStatus(values);
            afterCreate();
            handleClickClose();
        } catch (ex) {
        ...
        <>
            ...
            >
                <DialogTitleWithButton
                    title={t('pages.admin.orderStatus.createTitle')}
