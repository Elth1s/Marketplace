...
import { IProductStatusInfo } from '../types';
...
const ProductStatusTable = () => {
    ...
    const [orderBy, setOrderBy] = useState<keyof IProductStatusInfo>('id');

    const [selected, setSelected] = useState<readonly number[]>([]);

    const { SearchProductStatuses, DeleteProductStatuses } = useActions();
    ...
    const getData = async () => {
        try {
            await SearchProductStatuses(page, rowsPerPage, name, isAscOrder, orderBy);
            setSelected([]);
        } catch (ex) {
        }
    };
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (selected.length == 0) {
            const newSelecteds = productStatuses.map((n) => n.id);
    ...
    return (
        <>
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", my: "30px" }}>
                <Typography variant="h1">Product statuses</Typography>
