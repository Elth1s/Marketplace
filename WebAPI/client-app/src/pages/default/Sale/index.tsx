...
const Sale = () => {
    ...
    const { GetCategoriesBySaleId, GetFiltersByCategoryIdForUser, SaleProducts, GetSaleByIdForUser, ResetCatalogFilters } = useActions();
    const { products, countProducts, filterNames, searchCatalog } = useTypedSelector(state => state.catalog);
    const { selectedSaleForUser } = useTypedSelector(state => state.sale);
    ...
    const getData = async () => {
        ...
        try {
            ...
            await SaleProducts(saleId, page, rowsPerPage, categories, filters)
    ...
    const getCategories = async () => {
        if (!saleId)
            return;
        try {
            await GetCategoriesBySaleId(saleId);
            await GetSaleByIdForUser(saleId);
        } catch (ex) {
        }
    };
    ...
    return (
        <>
            <Box sx={{ display: "flex" }}>
                <BoxFilterStyle>
                    <img
                        style={{ width: "100%", objectFit: "contain", paddingBottom: "15px" }}
                        src={selectedSaleForUser.verticalImage}
                        alt="saleImage"
                    />
                    <Box
                        sx={{
